...
- 再送は安全（冪等）: done済みへの`done`、openへの`reopen`、存在しないIDへの`rm` は `changed:false` の成功になる（このときの `task` は `null` になりうる）。exit 0なら再実行不要
- 失敗したらstderrのエラーJSONの `code` と `next_step` に従う。終了コード: 0=成功 / 1=io・internal / 2=usage / 3=not_found / 4=ambiguous / 5=validation
- タスク登録時、発話に背景情報（誰から・何のため・経緯等）が含まれていれば、タイトルとは別に `--note` で補足を入れる
